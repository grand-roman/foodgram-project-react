...
class FollowViewSet(APIView):
    ...
    def delete(self, request, user_id):
        user = request.user
        follow = get_object_or_404(Follow, user=user, author=user_id)
        follow.delete()
...
class FavouriteViewSet(APIView):
    ...
    def delete(self, request, recipe_id):
        user = request.user
        favorite_obj = get_object_or_404(Favorite, user=user, recipe=recipe_id)
        if not favorite_obj:
            return Response(
...
class ShoppingListViewSet(APIView):
    ...
    def get(self, request, recipe_id):
        user = request.user
        if ShoppingList.objects.filter(user=user, recipe=recipe_id).exists():
            return Response(
                'Вы уже добавили рецепт в список покупок',
                status=status.HTTP_400_BAD_REQUEST)
        ShoppingList.objects.create(user=user, recipe=recipe_id)
        serializer = AddFavouriteRecipeSerializer(recipe_id)
        return Response(serializer.data, status=status.HTTP_201_CREATED)

    def delete(self, request, recipe_id):
        user = request.user
        shopping_list_obj = get_object_or_404(
            ShoppingList, user=user, recipe=recipe_id)
